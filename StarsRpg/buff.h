...
struct Buff : public Effect { 
	...
	Buff(std::string name, CoreStats cs, EffectType duration = 1u, bool isdebuff = false)
	: Name(name), Duration(duration), isDebuff(isdebuff)
	{
		BuffedStats = cs;
	}
...
struct Debuff : public Effect { 
	...
	Debuff(std::string name, CoreStats cs, EffectType duration = 1u, bool isdebuff = false)
	: Name(name), Duration(duration), isDebuff(isdebuff)
	{
		DebuffedStats = cs;
	}
