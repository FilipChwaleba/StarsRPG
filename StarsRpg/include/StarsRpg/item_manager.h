...
public:
	...
	static bool Use(Item* item_to_use, PlayerCharacter* p_char) {
		...
		if (tmp_con) {
			...
			//jeśli hp czy stamina są max, lub gdy nie daje buffów
			//nie używać potek
			...
			//if (p_char->pcClass->Stamina->isFull() && !tmp_con->efc) 
				//return false;	
				// dopóki nie zrobie oddzielnych potek (Stamina i Energia), zakomentować

			//zwiększ HP lub/i Stamine, może być ustawione na zero
				p_char->pcClass->HP->increaseCurrent(tmp_con->well); 
				p_char->pcClass->Stamina->increaseCurrent(tmp_con->well); 

			//consumable został użyty
			tmp_con->Quantity--;
